...
func TestLocalRestoreStopsAutoTrip(t *testing.T) {
	bus := event.NewBus()
	...
	_ = sup.ManualRestore("p01")
	if fan.AutoEnabled() {
		t.Fatalf("manual restore must disable auto control for the fan")
	}
	if err := sup.Evaluate("p01"); err == nil {
		t.Fatalf("auto trip must yield while the manual hold is active")
	}
	if state, _ := sup.Table().Get("p01"); state.TripCount != 1 {
		t.Fatalf("trip count must stay one while manual hold is active, got %d", state.TripCount)
	}
}
