...
func (s *Supervisor) TriggerTrip(id string, timeout time.Duration) error {
	...
	if err := ctx.Err(); err != nil {
		...
		_ = s.cfg.Trips.Reset(id)
		_, _ = s.table.SetState(id, StateRestored)
		_ = s.cfg.Records.Append(record.Entry{
			Point:   id,
			Kind:    "trip_timeout",
			Message: err.Error(),
		...
		if lock, ok := s.cfg.Trips.State(id); ok {
			s.cfg.Bus.Publish("trip.timeout_reset", lock.State)
		}
		s.cfg.Bus.Publish("trip.timeout", id)
		return err
	}
	_ = <-fanDone
...
func (s *Supervisor) ConfirmStable(id string, window time.Duration, sample func() float64) error {
	...
	defer cancel()
	_ = vent.WaitStable(ctx, s.cfg.Thresholds.Alert, sample, s.cfg.SampleInterval)
